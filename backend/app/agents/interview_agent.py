...
from app.agents.context_utils import compact_planning
...
class InterviewAgent:
    async def run(self, planning: PlanningState, max_roles: int = 3, questions_per_role: int = 3) -> InterviewPlan:
        ...
        context = json.dumps(compact_planning(planning), indent=2)
        response = await get_llm_provider().generate(SYSTEM_PROMPT, INTERVIEW_PROMPT.format(planning_context=context), model=current_ai_model())
